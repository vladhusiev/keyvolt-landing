          ...
          <div className={styles.footer__nav}>
            ...
            <ul className={styles.footer__navList}>
              {menuItems.flatMap((item) => {
                // If item has children, include both parent and children as regular items
                if (item.children) {
                  return item.children.map((child) => {
                    const childHref = child.href || "#";
                    const isExternalLink = !childHref.startsWith("#");
                    const targetHref = isExternalLink
                      ? childHref
                      : `/${childHref}`;

                    return (
                      <li key={child.id} className={styles.footer__navItem}>
                        <Link
                          href={targetHref}
                  ...
                  const isExternalLink = !itemHref.startsWith("#");
                  const targetHref = isExternalLink ? itemHref : `/${itemHref}`;
