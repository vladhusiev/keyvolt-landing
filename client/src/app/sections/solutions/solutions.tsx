      ...
      <Container>
        ...
        <div className={styles.content}>
          <div className={styles.left}>
            <div className={styles.list}>
              {solutions?.map((item, idx) => (
                <button
                  key={idx}
                  className={`${styles.listItem} ${activeTab === idx ? styles.active : ""}`}
          ...
          <div className={styles.right}>
            {active && (
              <div className={`${styles.tabs} ${isFading ? styles.fading : ""}`}>
                <div className={styles.imageWrap}>
                  <Image src={active.image.url} alt={active.name} width={420} height={320} className={styles.image} />
                  <div className={styles.imageNote}>{active.main_text}</div>
                </div>
              </div>
            )}
          </div>
